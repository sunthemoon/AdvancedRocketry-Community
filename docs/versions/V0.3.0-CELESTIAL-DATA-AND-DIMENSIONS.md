...
- [x] 进入 Moon，检查重力基础效果、出生/安全平台策略、时间/天空占位。
- [x] 进入 Space，确认不会掉入无穷死亡循环；有安全返回命令。
- [x] 两玩家在不同维度，服务端状态和天体列表一致。
...
- [x] Earth/Moon/Space 使用 ResourceKey 稳定识别。
- [x] 坏天体数据不会静默生成错误世界。
- [x] SavedData 重启后准确恢复。
- [x] 固定维度专服可加载，无客户端类。
- [x] XML 导入输出可验证规范数据，未知字段明确列出。
- [x] 没有运行时动态维度或数字 ID 依赖。
- [x] 性能无每 tick 全天体重建/同步。
...
version: v0.3.0
status: IN_PROGRESS
commit: "63d159ef3d9e489862b0d517b76bcc523df852c9"
