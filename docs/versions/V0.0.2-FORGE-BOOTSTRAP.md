...
客户端验收使用已批准的准确实现提交 `d6c8464` 和主 JAR SHA-256
`cd5ae579...`。G0 来源/许可子审核、重建与跨平台逐字节一致性、完整可分发
源码/资源清单及历史审核、README 全窗口视觉审核均已完成，记录提交为
`db9ce96`。若后续修改打包字节、已审核源码/资源、来源记录或 README，必须
重新执行受影响的审核；不得把批准前或被变更取代的 JAR 作为客户端证据。

- [x] 运行只含 Forge 与最终发布 JAR 的隔离客户端，进入主菜单和 Mods 列表，确认名称、版本、描述和 credits。
- [x] 创建测试世界，确认没有 ERROR。
- [x] 启动专用服务端，接受测试 EULA 后完成启动、保存、停止和同世界重启。
- [x] 客户端连接专服，并原样记录缺少项目模组时“红色不兼容标记但连接成功”的实际行为。
- [x] 检查发布 JAR 名称和 GitHub artifact 不带 `NONE`/`unspecified` 版本。

匹配客户端和缺少项目 JAR 的客户端必须使用两个互不嵌套的隔离游戏目录；
schema-5 客户端证据记录绑定两边 `mods/` 的前后清单、准确 JAR/空清单和各自
目录内的原始日志；独立的 schema-4 人工玩家摘要绑定服务端生命周期。两者均
不接受复用同一 profile 或跨 profile 的物理日志文件。
...
- [x] 绑定原始日志无任何 ERROR，且无项目来源 WARN；可解释的非项目 WARN
      已记录并复核。
- [x] 不含任何未经审计上游资产或功能代码。
