#!/usr/bin/env python3
"""Bind two Forge userdev clients to one packaged v0.8 dedicated server."""
...
if __package__:
    ...
    from .run_v080_satellite_server_smoke import EVIDENCE_LOG, _report
else:
    from run_dedicated_server_smoke import (
        JOIN_MARKER,
        LEAVE_MARKER,
...
def _write_evidence(
    ...
    if directory.exists():
        raise SmokeError(f"Refusing to overwrite v0.8 multiplayer evidence: {directory}")
...
def main() -> int:
    ...
    try:
        ...
        process = harness.start("v080-two-client")
        ...
        authority = _report(process)
        if authority["satellites"] < 102 or authority["missions"] < 102:
            raise SmokeError(f"Clients joined without the packaged satellite authority: {authority}")
        marker = (
            "ARCE_V080_G4_SHARED_STATE players=2 satellites="
            f"{authority['satellites']} missions={authority['missions']} "
        ...
        server_lines = [
            ...
            if (
                "Advanced Rocketry: Community Edition" in line
                or JOIN_MARKER.search(line)
                or LEAVE_MARKER.search(line)
                or EVIDENCE_LOG.search(line)
                or marker in line
                or "Saved the game" in line
            )
        ]
        process = None
        ...
        summary = {
            ...
            "java": java_version,
            "limitations": [
                "Clients are Forge user-development launches from the tested commit; the server uses the byte-identified packaged JAR.",
                "Terminal interaction authority is covered by Forge GameTest; this run proves simultaneous join, sync, and clean disconnect.",
            ],
        }
        evidence = args.evidence_dir.resolve()
        _write_evidence(evidence, summary, server_lines, filtered_clients)
        print("[PASS] Two simultaneous Forge clients joined the packaged v0.8 server")
        print("[PASS] Both clients received the same satellite authority marker")
