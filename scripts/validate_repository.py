...
if __package__:
    ...
    from .validate_v030_release_evidence import validate_v030_release_evidence
    from .validate_v040_release_evidence import validate_v040_release_evidence
    from .validate_v050_release_evidence import validate_v050_release_evidence
    ...
    from .manage_v050_generated_manifest import verify as verify_v050_generated_manifest
...
REQUIRED_PATHS = (
    ...
    "scripts/collect_v090_compatibility_evidence.py",
    ...
    "scripts/manage_v040_generated_manifest.py",
    "scripts/manage_v050_generated_manifest.py",
    ...
    "scripts/validate_v050_release_evidence.py",
    ...
    "tests/test_collect_v090_compatibility_evidence.py",
    ...
    "tests/test_validate_v030_release_evidence.py",
    "tests/test_validate_v040_release_evidence.py",
    "tests/test_validate_v050_release_evidence.py",
    ...
    "tests/test_run_v060_flight_server_smoke.py",
    ...
    "docs/provenance/v0.4.0-generated-resources.json",
    "docs/provenance/v0.4.0-atmosphere.md",
    "docs/provenance/v0.5.0-generated-resources.json",
    "docs/provenance/v0.5.0-rocket-assembly.md",
    "docs/provenance/v0.6.0-generated-resources.json",
    "docs/provenance/v0.6.0-earth-moon-roundtrip.md",
    "docs/provenance/v0.7.0-generated-resources.json",
    ...
    "docs/decisions/ADR-007-V060-VISUAL-EVIDENCE-ATTESTATION.md",
    "docs/decisions/ADR-008-SHARED-SPACE-STATION-REGIONS.md",
    "docs/decisions/ADR-009-V070-VISUAL-EVIDENCE-ATTESTATION.md",
    "docs/decisions/ADR-010-BOUNDED-SATELLITE-MISSIONS.md",
    "docs/decisions/ADR-011-V080-VISUAL-EVIDENCE-SEQUENCE.md",
...
def validate_repository_workflow_text(text: str) -> list[str]:
    ...
    for command in (
        ...
        ("python", "scripts/manage_v040_generated_manifest.py", "verify"),
        (
            "python",
            "scripts/validate_v040_release_evidence.py",
            "--require-approved",
        ),
        ("python", "scripts/manage_v050_generated_manifest.py", "verify"),
        (
            "python",
            "scripts/validate_v050_release_evidence.py",
            "--require-approved",
...
def validate_forge_workflow_text(text: str) -> list[str]:
    ...
    satellite = jobs.get("satellite-acceptance")
    ...
    if satellite is None or not satellite.blocking:
        errors.append("enabled blocking satellite-acceptance job")
    else:
        if satellite.env != {
            "REVIEW_COMMIT": "${{ github.event.pull_request.head.sha || github.sha }}"
        }:
        ...
        if satellite.fields.get("timeout-minutes") != "20":
            errors.append("satellite-acceptance exact 20-minute job timeout")
        if not _job_has_exact_action_contract(
            satellite,
            "actions/checkout@v7",
            {
            ...
            errors.append("satellite-acceptance exact head-bound checkout contract")
        if not _job_has_exact_action_contract(
            satellite,
            "actions/setup-python@v7",
            {"python-version": "3.12"},
        ):
            errors.append("satellite-acceptance exact Python action contract")
        for action in (
            "actions/setup-java@v6",
            "gradle/actions/setup-gradle@v6",
        ):
            if not _job_has_action(satellite, action):
                errors.append(f"satellite-acceptance enabled action {action}")
        upload_steps = [
            step
            for step in _required_steps(satellite)
            if step.fields.get("uses", "").startswith("actions/upload-artifact@")
        ]
        if (
            len(upload_steps) != 1
        ...
        for command in satellite_commands:
            if not _job_has_command(satellite, command):
                errors.append(
                    "satellite-acceptance enabled run command " + " ".join(command)
                )
...
def check_v040_generated_resources(results: Results) -> None:
    errors = verify_v040_generated_manifest(
        ROOT,
        ROOT / "docs/provenance/v0.4.0-generated-resources.json",
    )
    if errors:
        results.fail("v0.4.0 generated-resource errors: " + "; ".join(errors))
    else:
        results.passed(
            "v0.4.0 DataGen resources match the exact atmosphere inventory"
        )


def check_v050_generated_resources(results: Results) -> None:
    errors = verify_v050_generated_manifest(
        ROOT,
        ROOT / "docs/provenance/v0.5.0-generated-resources.json",
    )
    if errors:
        results.fail("v0.5.0 generated-resource errors: " + "; ".join(errors))
    else:
        results.passed(
            "v0.5.0 DataGen resources match the exact rocket inventory"
        )


def check_v060_generated_resources(results: Results) -> None:
    errors = verify_v060_generated_manifest(
        ROOT,
        ROOT / "docs/provenance/v0.6.0-generated-resources.json",
    )
    if errors:
        results.fail("v0.6.0 generated-resource errors: " + "; ".join(errors))
    else:
        results.passed(
            "v0.6.0 DataGen resources match the exact fuel inventory"
        )


def check_v070_generated_resources(results: Results) -> None:
    errors = verify_v070_generated_manifest(
        ROOT,
        ROOT / "docs/provenance/v0.7.0-generated-resources.json",
    )
    if errors:
        results.fail("v0.7.0 generated-resource errors: " + "; ".join(errors))
    else:
        results.passed(
            "v0.7.0 DataGen resources match the exact station inventory"
        )


def check_v080_generated_resources(results: Results) -> None:
    errors = verify_v080_generated_manifest(
        ROOT,
        ROOT / "docs/provenance/v0.8.0-generated-resources.json",
    )
    if errors:
        results.fail("v0.8.0 generated-resource errors: " + "; ".join(errors))
    else:
        results.passed(
...
def validate_v030_gate_status_text(
    text: str,
    *,
    evidence_details: dict[str, object] | None = None,
) -> list[str]:
    """Reject v0.3.0 Gate claims that exceed bound celestial evidence."""
    ...
    human_approved = bool(
        document_approval and evidence.get("human_approved") is True
    )
    if (reviewer or reviewed_at) and not human_approved:
        errors.append(
            "v0.3.0 human approval must match an authorized bound evidence review"
        )
    ...
    if waived:
        errors.append(
            "v0.3.0 Required Gates cannot be NOT_APPLICABLE: "
            + ", ".join(waived)
        )
    ...
    for gate, key in evidence_keys.items():
        if gates.get(gate) == "PASS" and evidence.get(key) is not True:
            errors.append(
                f"{gate} cannot be PASS without bound v0.3.0 {key} evidence"
            )
    ...
    overall_passed = top.get("overall") in {"PASS", "PASSED"}
    if status_passed != overall_passed:
        errors.append("v0.3.0 status PASSED and overall PASS/PASSED must agree")
    status_ready = top.get("status") == "READY_FOR_AUDIT"
    overall_ready = top.get("overall") == "READY_FOR_AUDIT"
    if status_ready != overall_ready:
        errors.append(
            "v0.3.0 status and overall READY_FOR_AUDIT values must agree"
        )
    if status_ready:
        missing = [
        ...
        if missing:
            errors.append(
                "v0.3.0 cannot be READY_FOR_AUDIT without all technical evidence: "
                + ", ".join(missing)
            )
    if status_passed or overall_passed:
        unresolved = [gate for gate in required if gates.get(gate) != "PASS"]
        if unresolved:
            errors.append(
                "v0.3.0 cannot be PASSED while Required Gates are unresolved: "
                + ", ".join(unresolved)
            )
        if not human_approved or any(
            ...
            errors.append(
                "v0.3.0 cannot be PASSED without all bound evidence and human approval"
            )
    return errors


def check_v030_gate_status(results: Results) -> None:
    current_version = read_text(ROOT / "docs/status/CURRENT_VERSION.md", results)
    historical = ROOT / "docs/releases/v0.3.0/GATE-STATUS.md"
    is_current = "current_version: v0.3.0" in current_version
    if not is_current and not historical.exists():
        return

    release_errors, evidence_details = validate_v030_release_evidence(
        repository_root=ROOT
    )
    if release_errors:
        results.fail(
            "v0.3.0 release evidence errors: " + "; ".join(release_errors)
        )
        evidence_details = {}
    else:
        results.passed(
            "v0.3.0 artifact, celestial, persistence, multiplayer, client, and checksum evidence is valid"
        )
    ...
    for document in documents:
        ...
        if errors:
            results.fail(
                f"v0.3.0 Gate status contradictions in {document.relative_to(ROOT)}: "
                + "; ".join(errors)
            )
        else:
            results.passed(
                f"v0.3.0 Gate status is evidence-bound in {document.relative_to(ROOT)}"
            )


def validate_v040_gate_status_text(
    text: str,
    *,
    evidence_details: dict[str, object] | None = None,
) -> list[str]:
    """Reject v0.4.0 Gate claims that exceed bound atmosphere evidence."""
    ...
    if (reviewer or reviewed_at) and not human_approved:
        errors.append(
            "v0.4.0 human approval must match an authorized bound evidence review"
        )
    ...
    if waived:
        errors.append(
            "v0.4.0 Required Gates cannot be NOT_APPLICABLE: "
            + ", ".join(waived)
        )
    ...
    for gate, key in evidence_keys.items():
        if gates.get(gate) == "PASS" and evidence.get(key) is not True:
            errors.append(
                f"{gate} cannot be PASS without bound v0.4.0 {key} evidence"
            )
    for gate in ("G0", "G8", "G9"):
        if gates.get(gate) == "READY_FOR_HUMAN_REVIEW":
            key = evidence_keys[gate]
    ...
    overall_passed = top.get("overall") in {"PASS", "PASSED"}
    if status_passed != overall_passed:
        errors.append("v0.4.0 status PASSED and overall PASS/PASSED must agree")
    status_ready = top.get("status") == "READY_FOR_AUDIT"
    overall_ready = top.get("overall") == "READY_FOR_AUDIT"
    if status_ready != overall_ready:
        errors.append(
            "v0.4.0 status and overall READY_FOR_AUDIT values must agree"
        )
    if status_ready:
        missing = [
            key for key in evidence_keys.values() if evidence.get(key) is not True
        ...
        if missing:
            errors.append(
                "v0.4.0 cannot be READY_FOR_AUDIT without all technical evidence: "
                + ", ".join(missing)
            )
    if status_passed or overall_passed:
        unresolved = [gate for gate in required if gates.get(gate) != "PASS"]
        if unresolved:
            errors.append(
                "v0.4.0 cannot be PASSED while Required Gates are unresolved: "
                + ", ".join(unresolved)
            )
        if not human_approved or any(
            evidence.get(key) is not True for key in evidence_keys.values()
            ...
            errors.append(
                "v0.4.0 cannot be PASSED without all bound evidence and human approval"
            )
    return errors


def check_v040_gate_status(results: Results) -> None:
    current_version = read_text(ROOT / "docs/status/CURRENT_VERSION.md", results)
    historical = ROOT / "docs/releases/v0.4.0/GATE-STATUS.md"
    is_current = "current_version: v0.4.0" in current_version
    if not is_current and not historical.exists():
        return

    release_errors, evidence_details = validate_v040_release_evidence(
        repository_root=ROOT
    )
    if release_errors:
        results.fail(
            "v0.4.0 release evidence errors: " + "; ".join(release_errors)
        )
        evidence_details = {}
    else:
        results.passed(
            "v0.4.0 artifact, atmosphere, persistence, performance, multiplayer, client, and checksum evidence is valid"
        )
    ...
    for document in documents:
        text = read_text(document, results)
        errors = validate_v040_gate_status_text(
            text,
            evidence_details=evidence_details,
        )
        if errors:
            results.fail(
                f"v0.4.0 Gate status contradictions in {document.relative_to(ROOT)}: "
                + "; ".join(errors)
            )
        else:
            results.passed(
                f"v0.4.0 Gate status is evidence-bound in {document.relative_to(ROOT)}"
            )


def validate_v050_gate_status_text(
    text: str,
    *,
    evidence_details: dict[str, object] | None = None,
) -> list[str]:
    """Reject v0.5.0 Gate claims that exceed bound rocket evidence."""
    ...
    if (reviewer or reviewed_at) and not human_approved:
        errors.append(
            "v0.5.0 human approval must match an authorized bound evidence review"
        )
    ...
    if waived:
        errors.append(
            "v0.5.0 Required Gates cannot be NOT_APPLICABLE: "
            + ", ".join(waived)
        )
    ...
    for gate, key in evidence_keys.items():
        if gates.get(gate) == "PASS" and evidence.get(key) is not True:
            errors.append(
                f"{gate} cannot be PASS without bound v0.5.0 {key} evidence"
            )
    for gate in ("G0", "G8", "G9"):
        if gates.get(gate) == "READY_FOR_HUMAN_REVIEW":
            key = evidence_keys[gate]
    ...
    overall_passed = top.get("overall") in {"PASS", "PASSED"}
    if status_passed != overall_passed:
        errors.append("v0.5.0 status PASSED and overall PASS/PASSED must agree")
    status_ready = top.get("status") == "READY_FOR_AUDIT"
    overall_ready = top.get("overall") == "READY_FOR_AUDIT"
    if status_ready != overall_ready:
        errors.append(
            "v0.5.0 status and overall READY_FOR_AUDIT values must agree"
        )
    if status_ready:
        missing = [
            key for key in evidence_keys.values() if evidence.get(key) is not True
        ...
        if missing:
            errors.append(
                "v0.5.0 cannot be READY_FOR_AUDIT without all technical evidence: "
                + ", ".join(missing)
            )
    if status_passed or overall_passed:
        unresolved = [gate for gate in required if gates.get(gate) != "PASS"]
        if unresolved:
            errors.append(
                "v0.5.0 cannot be PASSED while Required Gates are unresolved: "
                + ", ".join(unresolved)
            )
        if not human_approved or any(
            evidence.get(key) is not True for key in evidence_keys.values()
            ...
            errors.append(
                "v0.5.0 cannot be PASSED without all bound evidence and human approval"
            )
    return errors


def check_v050_gate_status(results: Results) -> None:
    current_version = read_text(ROOT / "docs/status/CURRENT_VERSION.md", results)
    historical = ROOT / "docs/releases/v0.5.0/GATE-STATUS.md"
    is_current = "current_version: v0.5.0" in current_version
    if not is_current and not historical.exists():
        return

    release_errors, evidence_details = validate_v050_release_evidence(
        repository_root=ROOT
    )
    if release_errors:
        results.fail(
            "v0.5.0 release evidence errors: " + "; ".join(release_errors)
        )
        evidence_details = {}
    else:
        results.passed(
            "v0.5.0 artifact, rocket, persistence, performance, client, and checksum evidence is valid"
        )
    ...
    for document in documents:
        text = read_text(document, results)
        errors = validate_v050_gate_status_text(
            text,
            evidence_details=evidence_details,
        )
        if errors:
            results.fail(
                f"v0.5.0 Gate status contradictions in {document.relative_to(ROOT)}: "
                + "; ".join(errors)
            )
        else:
            results.passed(
                f"v0.5.0 Gate status is evidence-bound in {document.relative_to(ROOT)}"
            )


def validate_v060_gate_status_text(
    text: str,
    *,
    evidence_details: dict[str, object] | None = None,
) -> list[str]:
    """Reject v0.6.0 Gate claims that exceed bound flight evidence."""
    ...
    if (reviewer or reviewed_at) and not human_approved:
        errors.append(
            "v0.6.0 human approval must match an authorized bound evidence review"
        )
    ...
    if waived:
        errors.append(
            "v0.6.0 Required Gates cannot be NOT_APPLICABLE: "
            + ", ".join(waived)
        )
    ...
    for gate, key in evidence_keys.items():
        if gates.get(gate) == "PASS" and evidence.get(key) is not True:
            errors.append(
                f"{gate} cannot be PASS without bound v0.6.0 {key} evidence"
            )
    for gate in ("G0", "G8", "G9"):
        if gates.get(gate) == "READY_FOR_HUMAN_REVIEW":
            key = evidence_keys[gate]
    ...
    overall_passed = top.get("overall") in {"PASS", "PASSED"}
    if status_passed != overall_passed:
        errors.append("v0.6.0 status PASSED and overall PASS/PASSED must agree")
    status_ready = top.get("status") == "READY_FOR_AUDIT"
    overall_ready = top.get("overall") == "READY_FOR_AUDIT"
    if status_ready != overall_ready:
        errors.append(
            "v0.6.0 status and overall READY_FOR_AUDIT values must agree"
        )
    if status_ready:
        missing = [
            key for key in evidence_keys.values() if evidence.get(key) is not True
        ...
        if missing:
            errors.append(
                "v0.6.0 cannot be READY_FOR_AUDIT without all technical evidence: "
                + ", ".join(missing)
            )
    if status_passed or overall_passed:
        unresolved = [gate for gate in required if gates.get(gate) != "PASS"]
        if unresolved:
            errors.append(
                "v0.6.0 cannot be PASSED while Required Gates are unresolved: "
                + ", ".join(unresolved)
            )
        if not human_approved or any(
            evidence.get(key) is not True for key in evidence_keys.values()
            ...
            errors.append(
                "v0.6.0 cannot be PASSED without all bound evidence and human approval"
            )
    return errors


def check_v060_gate_status(results: Results) -> None:
    current_version = read_text(ROOT / "docs/status/CURRENT_VERSION.md", results)
    historical = ROOT / "docs/releases/v0.6.0/GATE-STATUS.md"
    is_current = "current_version: v0.6.0" in current_version
    if not is_current and not historical.exists():
        return

    release_errors, evidence_details = validate_v060_release_evidence(
        repository_root=ROOT
    )
    if release_errors:
        results.fail(
            "v0.6.0 release evidence errors: " + "; ".join(release_errors)
        )
        evidence_details = {}
    else:
        results.passed(
            "v0.6.0 artifact, flight, recovery, multiplayer, performance, and checksum evidence is valid"
        )
    ...
    for document in documents:
        text = read_text(document, results)
        errors = validate_v060_gate_status_text(
            text,
            evidence_details=evidence_details,
        )
        if errors:
            results.fail(
                f"v0.6.0 Gate status contradictions in {document.relative_to(ROOT)}: "
                + "; ".join(errors)
            )
        else:
            results.passed(
                f"v0.6.0 Gate status is evidence-bound in {document.relative_to(ROOT)}"
            )


def validate_v070_gate_status_text(
    text: str,
    *,
    evidence_details: dict[str, object] | None = None,
) -> list[str]:
    """Reject v0.7.0 Gate claims that exceed bound station evidence."""
    ...
    if (reviewer or reviewed_at) and not human_approved:
        errors.append(
            "v0.7.0 human approval must match an authorized bound evidence review"
        )
    ...
    if waived:
        errors.append(
            "v0.7.0 Required Gates cannot be NOT_APPLICABLE: "
            + ", ".join(waived)
        )
    ...
    for gate, key in evidence_keys.items():
        if gates.get(gate) == "PASS" and evidence.get(key) is not True:
            errors.append(
                f"{gate} cannot be PASS without bound v0.7.0 {key} evidence"
            )
    for gate in ("G0", "G8", "G9"):
        if gates.get(gate) == "READY_FOR_HUMAN_REVIEW":
            key = evidence_keys[gate]
    ...
    overall_passed = top.get("overall") in {"PASS", "PASSED"}
    if status_passed != overall_passed:
        errors.append("v0.7.0 status PASSED and overall PASS/PASSED must agree")
    status_ready = top.get("status") == "READY_FOR_AUDIT"
    overall_ready = top.get("overall") == "READY_FOR_AUDIT"
    if status_ready != overall_ready:
        errors.append(
            "v0.7.0 status and overall READY_FOR_AUDIT values must agree"
        )
    if status_ready:
        missing = [
            key for key in evidence_keys.values() if evidence.get(key) is not True
        ...
        if missing:
            errors.append(
                "v0.7.0 cannot be READY_FOR_AUDIT without all technical evidence: "
                + ", ".join(missing)
            )
    if status_passed or overall_passed:
        unresolved = [gate for gate in required if gates.get(gate) != "PASS"]
        if unresolved:
            errors.append(
                "v0.7.0 cannot be PASSED while Required Gates are unresolved: "
                + ", ".join(unresolved)
            )
        if not human_approved or any(
            evidence.get(key) is not True for key in evidence_keys.values()
            ...
            errors.append(
                "v0.7.0 cannot be PASSED without all bound evidence and human approval"
            )
...
def check_v070_gate_status(results: Results) -> None:
    current_version = read_text(ROOT / "docs/status/CURRENT_VERSION.md", results)
    historical = ROOT / "docs/releases/v0.7.0/GATE-STATUS.md"
    is_current = "current_version: v0.7.0" in current_version
    if not is_current and not historical.exists():
        return

    release_errors, evidence_details = validate_v070_release_evidence(
        repository_root=ROOT
    )
    if release_errors:
        results.fail(
            "v0.7.0 release evidence errors: " + "; ".join(release_errors)
        )
        evidence_details = {}
    else:
        results.passed(
            "v0.7.0 artifact, station, recovery, multiplayer, performance, and checksum evidence is valid"
        )
    ...
    for document in documents:
        ...
        if errors:
            results.fail(
                f"v0.7.0 Gate status contradictions in {document.relative_to(ROOT)}: "
                + "; ".join(errors)
            )
        else:
            results.passed(
                f"v0.7.0 Gate status is evidence-bound in {document.relative_to(ROOT)}"
            )


def validate_v080_gate_status_text(
    text: str,
    *,
    evidence_details: dict[str, object] | None = None,
) -> list[str]:
    """Reject v0.8.0 Gate claims that exceed bound satellite evidence."""
    ...
    if (reviewer or reviewed_at) and not human_approved:
        errors.append(
            "v0.8.0 human approval must match an authorized bound evidence review"
        )
    ...
    if waived:
        errors.append(
            "v0.8.0 Required Gates cannot be NOT_APPLICABLE: "
            + ", ".join(waived)
        )
    ...
    for gate, key in evidence_keys.items():
        if gates.get(gate) == "PASS" and evidence.get(key) is not True:
            errors.append(
                f"{gate} cannot be PASS without bound v0.8.0 {key} evidence"
            )
    for gate in ("G0", "G8", "G9"):
        if gates.get(gate) == "READY_FOR_HUMAN_REVIEW":
            key = evidence_keys[gate]
    ...
    overall_passed = top.get("overall") in {"PASS", "PASSED"}
    if status_passed != overall_passed:
        errors.append("v0.8.0 status PASSED and overall PASS/PASSED must agree")
    status_ready = top.get("status") == "READY_FOR_AUDIT"
    overall_ready = top.get("overall") == "READY_FOR_AUDIT"
    if status_ready != overall_ready:
        errors.append(
            "v0.8.0 status and overall READY_FOR_AUDIT values must agree"
        )
    if status_ready:
        missing = [
            key for key in evidence_keys.values() if evidence.get(key) is not True
        ...
        if missing:
            errors.append(
                "v0.8.0 cannot be READY_FOR_AUDIT without all technical evidence: "
                + ", ".join(missing)
            )
    if status_passed or overall_passed:
        unresolved = [gate for gate in required if gates.get(gate) != "PASS"]
        if unresolved:
            errors.append(
                "v0.8.0 cannot be PASSED while Required Gates are unresolved: "
                + ", ".join(unresolved)
            )
        if not human_approved or any(
            evidence.get(key) is not True for key in evidence_keys.values()
            ...
            errors.append(
                "v0.8.0 cannot be PASSED without all bound evidence and human approval"
            )
        if evidence.get("post_merge_ready") is not True:
            errors.append(
                "v0.8.0 cannot be PASSED without exact post-merge reproduction"
            )
        bindings = {
            "reviewed_head_commit": "reviewed_head_commit",
            "merge_commit": "merge_commit",
        ...
        for document_key, evidence_key in bindings.items():
            if top.get(document_key) != evidence.get(evidence_key):
                errors.append(
                    "v0.8.0 PASSED status must bind "
                    f"{document_key} to post-merge evidence"
                )
    return errors


def check_v080_gate_status(results: Results) -> None:
    current_version = read_text(ROOT / "docs/status/CURRENT_VERSION.md", results)
    historical = ROOT / "docs/releases/v0.8.0/GATE-STATUS.md"
    is_current = "current_version: v0.8.0" in current_version
    if not is_current and not historical.exists():
        return

    release_errors, evidence_details = validate_v080_release_evidence(
        repository_root=ROOT
    )
    if release_errors:
        results.fail(
            "v0.8.0 release evidence errors: " + "; ".join(release_errors)
        )
        evidence_details = {}
    else:
        results.passed(
            "v0.8.0 artifact, satellite, restart, multiplayer, performance, and checksum evidence is valid"
        )
    ...
    for document in documents:
        ...
        if errors:
            results.fail(
                f"v0.8.0 Gate status contradictions in {document.relative_to(ROOT)}: "
                + "; ".join(errors)
            )
        else:
            results.passed(
                f"v0.8.0 Gate status is evidence-bound in {document.relative_to(ROOT)}"
            )
