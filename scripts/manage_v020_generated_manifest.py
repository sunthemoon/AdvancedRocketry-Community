#!/usr/bin/env python3
"""Generate or verify the bounded hash inventory for v0.2.0 DataGen resources."""
...
GENERATED_ROOT = Path("src/generated/v0.2/resources")
DEFAULT_OUTPUT = Path("docs/provenance/v0.2.0-generated-resources.json")
MAX_FILES = 128
...
def render_manifest(repository_root: Path = ROOT) -> dict[str, object]:
    ...
    if root.is_symlink() or not root.is_dir():
        raise ValueError("v0.2.0 generated resource root is missing or unsafe")
    ...
    for path in sorted(root.rglob("*"), key=lambda value: value.as_posix()):
        ...
        if path.suffix.lower() not in ALLOWED_SUFFIXES:
            raise ValueError(f"v0.2.0 generated target has an unapproved type: {relative}")
...
def verify(repository_root: Path, output: Path) -> list[str]:
    ...
    errors: list[str] = []
    if actual != expected:
        errors.append("v0.2.0 generated-resource manifest does not match DataGen target bytes")
    if output.read_bytes() != _pretty(actual):
        errors.append("v0.2.0 generated-resource manifest is not canonical sorted JSON")
    return errors
...
def main() -> int:
    ...
    try:
        if args.mode == "generate":
            ...
            print(f"[PASS] Recorded {len(manifest['targets'])} v0.2.0 generated resources")
            return 0
    ...
    print(f"[PASS] v0.2.0 manifest matches {len(manifest['targets'])} DataGen files")
    return 0
