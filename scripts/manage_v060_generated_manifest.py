#!/usr/bin/env python3
"""Generate or verify the exact v0.6.0 DataGen resource inventory."""
...
GENERATED_ROOT = Path("src/generated/v0.6/resources")
DEFAULT_OUTPUT = Path("docs/provenance/v0.6.0-generated-resources.json")
MAX_FILE_BYTES = 2 * 1024 * 1024
MAX_TOTAL_BYTES = 16 * 1024 * 1024
EXPECTED_PATHS = frozenset(
    {
        "assets/advancedrocketrycommunity/blockstates/fuel_loader.json",
        "assets/advancedrocketrycommunity/models/block/fuel_loader.json",
        "assets/advancedrocketrycommunity/models/item/fuel_loader.json",
        "assets/advancedrocketrycommunity/models/item/rocket_fuel_cell.json",
        "assets/advancedrocketrycommunity_v060/lang/en_us.json",
        "assets/advancedrocketrycommunity_v060/lang/zh_cn.json",
        "data/advancedrocketrycommunity/advancements/recipes/misc/fuel_loader.json",
        "data/advancedrocketrycommunity/advancements/recipes/misc/rocket_fuel_cell.json",
        "data/advancedrocketrycommunity/loot_tables/blocks/fuel_loader.json",
        "data/advancedrocketrycommunity/recipes/fuel_loader.json",
        "data/advancedrocketrycommunity/recipes/rocket_fuel_cell.json",
    }
)
...
def render_manifest(repository_root: Path = ROOT) -> dict[str, object]:
    ...
    if root.is_symlink() or not root.is_dir():
        raise ValueError("v0.6.0 generated resource root is missing or unsafe")
    ...
    for path in sorted(root.rglob("*"), key=lambda value: value.as_posix()):
        ...
        if path.suffix.lower() != ".json":
            raise ValueError(f"v0.6.0 generated target has an unapproved type: {relative}")
        ...
        targets.append(
            {"path": relative, "bytes": size, "sha256": hashlib.sha256(data).hexdigest()}
        )
    ...
    if observed != EXPECTED_PATHS:
        ...
        raise ValueError(
            f"v0.6.0 generated inventory mismatch; missing={missing}, "
...
def verify(repository_root: Path, output: Path) -> list[str]:
    ...
    errors: list[str] = []
    if actual != expected:
        errors.append("v0.6.0 generated-resource manifest does not match DataGen target bytes")
    if output.read_bytes() != _pretty(actual):
        errors.append("v0.6.0 generated-resource manifest is not canonical sorted JSON")
    return errors
...
def main() -> int:
    ...
    try:
        if args.mode == "generate":
            ...
            print(f"[PASS] Recorded {len(manifest['targets'])} v0.6.0 generated resources")
            return 0
    ...
    print(f"[PASS] v0.6.0 manifest matches {len(manifest['targets'])} DataGen files")
    return 0
