#!/usr/bin/env python3
"""Generate or verify the exact v0.3.0 DataGen resource inventory."""
...
GENERATED_ROOT = Path("src/generated/v0.3/resources")
DEFAULT_OUTPUT = Path("docs/provenance/v0.3.0-generated-resources.json")
MAX_FILE_BYTES = 2 * 1024 * 1024
MAX_TOTAL_BYTES = 16 * 1024 * 1024
EXPECTED_PATHS = frozenset(
    {
        "data/advancedrocketrycommunity/celestial_bodies/earth.json",
        "data/advancedrocketrycommunity/celestial_bodies/moon.json",
        "data/advancedrocketrycommunity/celestial_bodies/space.json",
        "data/advancedrocketrycommunity/dimension/moon.json",
        "data/advancedrocketrycommunity/dimension/space.json",
        "data/advancedrocketrycommunity/dimension_type/moon.json",
        "data/advancedrocketrycommunity/dimension_type/space.json",
    }
)
...
def render_manifest(repository_root: Path = ROOT) -> dict[str, object]:
    ...
    if root.is_symlink() or not root.is_dir():
        raise ValueError("v0.3.0 generated resource root is missing or unsafe")
    ...
    for path in sorted(root.rglob("*"), key=lambda value: value.as_posix()):
        ...
        if path.suffix.lower() != ".json":
            raise ValueError(f"v0.3.0 generated target is not JSON: {relative}")
        size = path.stat().st_size
        if size > MAX_FILE_BYTES:
            raise ValueError(
                f"generated target exceeds {MAX_FILE_BYTES} bytes: {relative}"
            )
        total += size
    ...
    if observed != EXPECTED_PATHS:
        ...
        raise ValueError(
            f"v0.3.0 generated inventory mismatch; missing={missing}, "
...
def verify(repository_root: Path, output: Path) -> list[str]:
    ...
    if actual != expected:
        errors.append(
            "v0.3.0 generated-resource manifest does not match DataGen target bytes"
        )
    if output.read_bytes() != _pretty(actual):
        errors.append("v0.3.0 generated-resource manifest is not canonical sorted JSON")
    return errors
...
def main() -> int:
    ...
    try:
        if args.mode == "generate":
            ...
            print(
                f"[PASS] Recorded {len(manifest['targets'])} v0.3.0 "
                "generated resources"
            )
            return 0
    ...
    print(f"[PASS] v0.3.0 manifest matches {len(manifest['targets'])} DataGen files")
    return 0
