...
class V040GateStatusTests(unittest.TestCase):
    @staticmethod
    def complete_evidence(*, human_approved: bool = False) -> dict[str, object]:
        return {
            "provenance_ready": True,
    ...
    def test_required_gates_cannot_be_waived(self) -> None:
        errors = validate_v040_gate_status_text(
            v040_gate_document(gates={"G7": "NOT_APPLICABLE"})
        )
        self.assertTrue(any("G7" in error for error in errors))

    def test_ready_for_audit_requires_bound_technical_evidence(self) -> None:
        errors = validate_v040_gate_status_text(
            v040_gate_document(
                status="READY_FOR_AUDIT",
                overall="READY_FOR_AUDIT",
...
class V050GateStatusTests(unittest.TestCase):
    @staticmethod
    def complete_evidence(*, human_approved: bool = False) -> dict[str, object]:
        return {
            "provenance_ready": True,
    ...
    def test_required_gates_cannot_be_waived(self) -> None:
        errors = validate_v050_gate_status_text(
            v050_gate_document(gates={"G5": "NOT_APPLICABLE"})
        )
        self.assertTrue(any("G5" in error for error in errors), errors)

    def test_ready_for_audit_requires_bound_owner_review(self) -> None:
        errors = validate_v050_gate_status_text(
            v050_gate_document(
                status="READY_FOR_AUDIT",
                overall="READY_FOR_AUDIT",
...
class V060GateStatusTests(unittest.TestCase):
    @staticmethod
    def complete_evidence(*, human_approved: bool = False) -> dict[str, object]:
        return {
            "provenance_ready": True,
    ...
    def test_required_gates_cannot_be_waived(self) -> None:
        errors = validate_v060_gate_status_text(
            v060_gate_document(gates={"G5": "NOT_APPLICABLE"})
        )
        self.assertTrue(any("G5" in error for error in errors), errors)

    def test_pass_requires_bound_owner_review(self) -> None:
        errors = validate_v060_gate_status_text(
            v060_gate_document(
                status="PASSED",
                overall="PASSED",
